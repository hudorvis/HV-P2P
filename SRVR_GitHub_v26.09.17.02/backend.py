...
class HVP2PBackend(QObject):
    ...
    def __init__(self, version="26.09.17.02", smoke_test: bool = False):
        super().__init__()
        ...
        # Only a fresh full STATUS packet may establish current W1P motion/firmware health.
        # HELLO/PONG/HMI diagnostics can prove that a peer exists, but must never revive
        # stale pre-reboot authority/RS485 state.
    ...
    def _parse_w1p(self, line):
        ...
        if line.startswith("HELLO"):
            ...
            # deliberate Setup IP readdress reboot. Never carry a previous
            # session's authority/RS485-good state across that boundary: only
            # a fresh full STATUS from the new session may clear this hold.
            self._w1p_status_last_seen = 0.0
            self.winch_fw_authority_hold = True
            self.winch_fw_authority_state = "awaiting_status"
        ...
        # Parse each full STATUS as a fresh safety transaction. Invalidate the
        # previous session/sample first; freshness is restored only after all
        # safety-critical fields and RS485 health have parsed successfully.
        self._w1p_status_last_seen = 0.0
        self.winch_fw_authority_hold = True
        self.winch_fw_authority_state = "invalid_status"
        self._w1p_internal_safety = True
        self.winch_rs_status = "Disconnected"
        fields = {}
        ...
        try:
            ...
            # Only a completely parsed STATUS is allowed to become the current
            # motion/safety sample for this W1P session.
            self._w1p_status_last_seen = time.time()
        except Exception as exc:
            self._log(f"[W1P] Rejected malformed STATUS; safety hold retained: {exc}")
    ...
    def _request_w1p_readdress(self, new_ip: str, timeout_s: float = 2.2) -> bool:
        ...
        if self.smoke_test:
            return True
        if not self._w1p_status_fresh():
            self._log(f"[Config] W1P IP change refused: current W1P {old_ip} has no fresh full STATUS")
            return False
    ...
    def _build_controller_display_packet(self) -> str:
        ...
        w1p_ok = bool(self._w1p_status_fresh())
        if self.position_source == "Virtual":
            w1p_state = "demo"
        elif not w1p_ok:
            w1p_state = "error"
    ...
    def _w1p_status_fresh(self) -> bool:
        """True only when this session has supplied a recent full W1P STATUS packet."""
        return bool(
            self.w1p.connected
            and self._w1p_status_last_seen > 0.0
            and (time.time() - self._w1p_status_last_seen) <= WINCH_STATUS_TIMEOUT_S
        )
    ...
    @Property(bool, notify=stateChanged)
    def rs485Connected(self): return bool(self._w1p_status_fresh() and self.winch_rs_status == "Connected")
    @Property(float, notify=stateChanged)
