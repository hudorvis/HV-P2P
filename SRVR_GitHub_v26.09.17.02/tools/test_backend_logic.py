...
os.environ["XDG_CONFIG_HOME"] = TMP_HOME.name
...
from backend import (
    ...
    WINCH_STATUS_TIMEOUT_S,
)

app = QCoreApplication.instance() or QCoreApplication([])
b = HVP2PBackend(version="26.09.17.02", smoke_test=True)
...
try:
    ...
    # Firmware-authority fail-closed contract: make every other W1P health field
    # explicitly good, then prove that FW_MATCH alone controls this authority hold.
    healthy_tail = "RS_STAT=CONNECTED LEAD_CFG=OK MODBUS=1 READY=1 POS_READ=1 IO_READ=1 DO1_CFG=1 DO4_CFG=1 SRDY=1"
    legacy_status = ("STATUS POS_M=0 VEL_MPS=0 IP=172.20.1.102 WRITE_EN=0 SW_SRVON=0 "
                     "VEL_WD=0 SERVICE_LOCK=0 " + healthy_tail)
    matched_status = ("STATUS POS_M=0 VEL_MPS=0 IP=172.20.1.102 WRITE_EN=0 SW_SRVON=0 "
                      "VEL_WD=0 SERVICE_LOCK=0 FW_MATCH=1 FW_AUTH=matched " + healthy_tail)
    b._parse_w1p(legacy_status)
    assert b.winch_rs_status == "Connected", "legacy fixture did not isolate firmware-authority state"
    assert b.winch_fw_authority_hold and b._w1p_internal_safety, "missing FW_MATCH did not fail closed"
    b._parse_w1p(matched_status)
    assert b.w1p_reported_ip == "172.20.1.102"
    assert b.winch_rs_status == "Connected" and not b.winch_fw_authority_hold and not b._w1p_internal_safety
    assert b._w1p_status_fresh(), "fresh matched W1P STATUS not accepted as current-session health"
    ...
    b._parse_w1p("HELLO NAME=HV_P2P_W1P VER=v26.09.17.02")
    ...
    b._w1p_status_last_seen = time.time() - WINCH_STATUS_TIMEOUT_S - 0.1
    assert not b._w1p_status_fresh(), "non-STATUS W1P traffic revived stale motion health"
    b._parse_w1p(matched_status)
    assert b._w1p_status_fresh() and not b.winch_fw_authority_hold

    # Malformed STATUS must never become a fresh safety sample or retain a
    # previous matched/RS485-good state. A later valid STATUS restores health.
    malformed_status = ("STATUS POS_M=not-a-number VEL_MPS=0 IP=172.20.1.102 WRITE_EN=0 SW_SRVON=0 "
                        "VEL_WD=0 SERVICE_LOCK=0 FW_MATCH=1 FW_AUTH=matched " + healthy_tail)
    b._parse_w1p(malformed_status)
    assert not b._w1p_status_fresh() and b.winch_fw_authority_hold and b._w1p_internal_safety
    assert b.winch_rs_status == "Disconnected", "malformed STATUS retained stale RS485-good state"
    b._parse_w1p(matched_status)
    assert b._w1p_status_fresh() and not b.winch_fw_authority_hold and b.winch_rs_status == "Connected"
    ...
    # v26.09.17.02 Virtual Position Source is a true SRVR demo mode. Setup must
    ...
    b._parse_w1p("STATUS POS_M=12.345 VEL_MPS=-4.5 IP=172.20.1.102 WRITE_EN=0 SW_SRVON=0 VEL_WD=0 SERVICE_LOCK=0 FW_MATCH=1 FW_AUTH=matched RS_STAT=CONNECTED LEAD_CFG=OK FW=v26.09.17.02")
    ...
    b2 = HVP2PBackend(version="26.09.17.02", smoke_test=True)
    try:
