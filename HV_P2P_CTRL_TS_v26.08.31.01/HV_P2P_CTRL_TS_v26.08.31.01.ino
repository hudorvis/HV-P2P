...
#define CTRL_TS_SEMVER "v26.08.31.01"
...
// Safe v26.08.31.01 approach: no backlight/brightness writes. This page only
...
static void screen_keepalive(){
  // v26.08.31.01: no periodic full-screen or left-strip invalidation or brightness writes.
...
static void set_label_text_if_changed(lv_obj_t *lbl, const char *txt){
  ...
  // v26.08.31.01: label-only invalidation. Parent/full-strip invalidation can
...
static void confirm_aux_idx(int idx, bool send_command){
  ...
  g_selected_aux_ms = 0;
  confirmed_aux = idx;
  clear_confirm_at = now_ms + 2000;  // v26.08.31.01: confirmed AUX tile stays lit for 2 seconds
...
static void style_estop_pill(bool active){
  // v26.08.31.01: the middle status banner follows the SRVR-resolved state.
...
static void refresh_status_ui(){
  ...
  // v26.08.31.01: do not turn the main middle box red purely because the
...
static void apply_hmi_packet(const String &line){
  ...
  // v26.08.31.01: if SRVR sends explicit status/status_level, trust it as
  ...
  // v26.08.31.01: no left-strip/full-screen invalidation on packets; progress marker animates locally.
}
...
static void apply_layout_packet(const String &line){
  ...
  if(lbl_subtitle) set_label_text_if_changed(lbl_subtitle, "v26.08.31.01");
...
static String sha256_hex(const uint8_t digest[32]){
  ...
  char out[65];
  for(size_t i=0;i<32;i++){ out[i*2]=HEX[digest[i]>>4]; out[i*2+1]=HEX[digest[i]&0x0F]; }
  out[64]='\0';
  return String(out);
}
...
static void fw_handle_block(const HVP2PRS485::Frame &frame){
  ...
  const size_t wrote = Update.write(frame.payload + 4, dataLen);
  if(wrote != dataLen){
...
static void create_ui(){
  ...
  lbl_subtitle=make_label(frame,"v26.08.31.01",690,21,&lv_font_montserrat_10,lv_color_hex(C_MUTED),92);
  ...
  ramp_l=make_panel(travel_panel,BAR_LIMIT_LEFT,51,0,7,0x687074,0x687074,0,LV_OPA_35); lv_obj_set_style_border_width(ramp_l,0,0);
  ramp_r=make_panel(travel_panel,BAR_LIMIT_RIGHT,51,0,7,0x687074,0x687074,0,LV_OPA_35); lv_obj_set_style_border_width(ramp_r,0,0);
...
static void service_link_state(){
  ...
  if(!link_alive) {
    // v26.08.31.01: local UART/display timeout is a CTRL-TS link warning, not
