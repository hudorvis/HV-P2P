...
firmware_authority = read("firmware_authority.py")
...
# Python syntax.
for rel in ("main.py", "backend.py", "firmware_authority.py"):
    try:
...
require('"firmware_authority.py"' in project_text, "SRVR project manifest must include firmware_authority.py")
require('FirmwareAuthorityService' in main and 'firmware_service.start()' in main and 'return 4' in main,
        "SRVR firmware authority must start fail-closed before normal runtime")
require('validate_firmware_bundle' in firmware_authority and 'EDGEBOX_ESP100' in firmware_authority and
        '/firmware/{role.lower()}' in firmware_authority,
        "SRVR firmware authority immutable role/target validation is missing")
...
require('#62d64d' not in all_qml and '#55cf52' not in all_qml and '#4a5054' not in all_qml,
        "near-duplicate legacy palette tokens remain in QML")
require('#63d84e' in all_qml and '#72ed21' in all_qml and '#26d5ff' in all_qml and '#4a4f52' in all_qml,
        "shared HV P2P palette tokens missing from QML")
require('W1P-TS Link' not in qml_setup and 'W1P-TS AUX ASSIGN' not in qml_setup,
        "obsolete W1P-TS controls remain visible in Setup")
...
# v26.09.15.01 locked Run/Setup revision. Keep the approved panel geometry and
...
require('profileValue(Number(gp.x), key)' in span_qml and
...
# v26.09.15.01 integration contract: fifth CTRL-TS AUX travels in the spare A7
...
require("cp main.py backend.py firmware_authority.py resources.qrc HV_P2P_SRVR.pyproject" in workflow,
        "workflow staging allow-list missing core files")
require('SRVR_FIRMWARE_BUNDLE' in workflow and 'firmware_bundle' in workflow,
        "workflow staging allow-list missing immutable SRVR firmware bundle")
...
require(all(token in workflow for token in ('CFBundleIdentifier', 'com.hvp2p.srvr', 'CFBundleShortVersionString', 'CFBundleVersion', 'HVP2PReleaseVersion', "BUNDLE_BUILD_VERSION: '2609.15.1'")),
...
require('runner: macos-15-intel' in workflow and 'runner: macos-15' in workflow and 'runs-on: windows-2025' in workflow,
...
require('--include-data-files=firmware_bundle/ctrl.bin=firmware_bundle/ctrl.bin' in workflow and
        '--include-data-files=firmware_bundle/w1p.bin=firmware_bundle/w1p.bin' in workflow and
        'patch_pyside_deploy_spec.py' in workflow,
...
require('HV-P2P-SRVR-v26.09.15.01-macOS-Intel' in workflow and
        'HV-P2P-SRVR-v26.09.15.01-macOS-Apple-Silicon' in workflow and
        'HV-P2P-SRVR-v26.09.15.01-Windows-x64' in workflow,
...
require('path_text[2] == ":"' in backend and 'parsed.netloc' in backend,
        "Windows drive-letter/UNC FileDialog path handling is missing")
