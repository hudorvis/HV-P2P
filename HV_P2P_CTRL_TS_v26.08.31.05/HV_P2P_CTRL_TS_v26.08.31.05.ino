...
#define CTRL_TS_SEMVER "v26.08.31.05"
...
// Safe v26.08.31.05 approach: no backlight/brightness writes. This page only
...
static void screen_keepalive(){
  // v26.08.31.05: no periodic full-screen or left-strip invalidation or brightness writes.
...
static void set_label_text_if_changed(lv_obj_t *lbl, const char *txt){
  ...
  // v26.08.31.05: label-only invalidation. Parent/full-strip invalidation can
...
static void confirm_aux_idx(int idx, bool send_command){
  ...
  g_selected_aux_ms = 0;
  confirmed_aux = idx;
  clear_confirm_at = now_ms + 2000;  // v26.08.31.05: confirmed AUX tile stays lit for 2 seconds
...
static void style_estop_pill(bool active){
  // v26.08.31.05: the middle status banner follows the SRVR-resolved state.
...
static void refresh_status_ui(){
  ...
  // v26.08.31.05: do not turn the main middle box red purely because the
...
static void apply_hmi_packet(const String &line){
  ...
  // v26.08.31.05: if SRVR sends explicit status/status_level, trust it as
  ...
  // v26.08.31.05: no left-strip/full-screen invalidation on packets; progress marker animates locally.
}
...
static void apply_layout_packet(const String &line){
  ...
  if(lbl_subtitle) set_label_text_if_changed(lbl_subtitle, "v26.08.31.05");
...
static void create_ui(){
  ...
  lbl_subtitle=make_label(frame,"v26.08.31.05",690,21,&lv_font_montserrat_10,lv_color_hex(C_MUTED),92);
...
static void service_link_state(){
  ...
  if(!link_alive) {
    // v26.08.31.05: local UART/display timeout is a CTRL-TS link warning, not
