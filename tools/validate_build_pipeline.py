...
hmi=(ROOT/'HV_P2P_CTRL_TS_v26.09.14.01/HV_P2P_CTRL_TS_v26.09.14.01.ino').read_text()
embed=(ROOT/'tools/embed_ctrl_ts_firmware.py').read_text()
guard=(ROOT/'HV_P2P_CTRL_EDGEBOX_v26.09.14.01/HV_P2P_CTRL_TS_Firmware_Image.h').read_text()
w1pp=(ROOT/'HV_P2P_W1P_EDGEBOX_v26.09.14.01/partitions.csv').read_text()
checks={
 ...
 'firmware artifact upload present': 'HV-P2P-v26.09.14.01-Native-Firmware' in wf,
 ...
 'SRVR frozen builds explicitly include firmware bundle data': wf.count('--include-data-dir=firmware_bundle=firmware_bundle') >= 5,
 'SRVR macOS Intel job present': 'runner: macos-15-intel' in wf and 'HV-P2P-SRVR-v26.09.14.01-macOS-Intel' in wf and 'expected_arch: x86_64' in wf,
 'SRVR macOS Apple Silicon job present': 'runner: macos-15' in wf and 'HV-P2P-SRVR-v26.09.14.01-macOS-Apple-Silicon' in wf and 'expected_arch: arm64' in wf,
 'SRVR Windows x64 job present': 'runs-on: windows-2025' in wf and 'HV-P2P-SRVR-v26.09.14.01-Windows-x64' in wf and 'expected AMD64/x64 executable' in wf,
 ...
 'Windows dependency-tool downloads are noninteractive': "'--assume-yes-for-downloads'" in wf and 'generated Nuitka command is missing --assume-yes-for-downloads' in wf and "for flag in ('--assume-yes-for-downloads'" in wf,
 ...
 'complete matched artifact present': 'HV-P2P-v26.09.14.01-Complete-Release' in wf and 'needs: [build-firmware, build-srvr-macos, build-srvr-windows]' in wf,
 'SRVR bundle metadata pinned': all(x in wf for x in ('BUNDLE_IDENTIFIER', 'com.hvp2p.srvr', "BUNDLE_SHORT_VERSION: '26.9.14'", "BUNDLE_BUILD_VERSION: '2609.14.1'", "APP_VERSION: '26.09.14.01'", 'HVP2PReleaseVersion')),
 'complete release preserves all original SRVR ZIPs': 'three untouched native ZIPs' in wf and 'HV P2P SRVR v26.09.14.01 macOS Intel.zip' in wf and 'HV P2P SRVR v26.09.14.01 macOS Apple Silicon.zip' in wf and 'HV P2P SRVR v26.09.14.01 Windows x64.zip' in wf,
 ...
 'Windows native config path regression present': 'LOCALAPPDATA' in (ROOT/'SRVR_GitHub_v26.09.14.01'/'backend.py').read_text() and '_dialog_path' in (ROOT/'SRVR_GitHub_v26.09.14.01'/'backend.py').read_text(),
 ...
 'SRVR authority module included in project manifest': 'firmware_authority.py' in (ROOT/'SRVR_GitHub_v26.09.14.01'/'HV_P2P_SRVR.pyproject').read_text(),
 'complete release internal SHA256 manifest': "manifest=root/'SHA256SUMS.txt'" in wf and "sha256(p)" in wf,
 'complete release external SHA256': "Path(str(out)+'.sha256')" in wf and 'HV P2P v26.09.14.01 Complete Release.zip.sha256' in wf,
}
