...
checks=[
    ...
    ['python3','-m','py_compile',str(ROOT/'SRVR_GitHub_v26.08.31.03'/'backend.py'),str(ROOT/'SRVR_GitHub_v26.08.31.03'/'main.py')],
    ['python3',str(ROOT/'SRVR_GitHub_v26.08.31.03'/'tools'/'validate_project.py')],
]
for cmd in checks:
    print('\n==>', ' '.join(cmd), flush=True)
    subprocess.run(cmd,check=True,cwd=ROOT)
print('\nALL_SOURCE_CHECKS_PASS')
print('NOTE: Native Arduino compilation is performed by .github/workflows/firmware-build.yml; physical RS485/EL7/motion tests remain bench commissioning gates.')
