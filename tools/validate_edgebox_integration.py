...
SRVR_DIR = ROOT / 'SRVR_GitHub_v26.09.14.02'
...
# Safety-facing functions intentionally extended in v26.09.14.02 are hash-locked separately below.
expected_w1p={
...
# v26.09.14.02 intentionally adds only the firmware-authority inhibit to the
...
for fn,h in expected_w1p_v14_safety.items(): must(normalized_func_hash(w,fn)==h, f'W1P v26.09.14.02 reviewed safety/authority hash locked: {fn}')
...
# v26.09.14.02 independent W1P command-deadman and service safety gate.
...
# v26.09.14.02 SRVR-authoritative startup/safe-idle firmware gate.  These are
...
# v26.09.14.02 closes the W1P Setup-IP semantic gap with a coordinated safe
...
# v26.09.14.02 locked Run/Setup revision and Virtual demo-source contract.
...
must('ctrl_version=v26.09.14.02' in c and 'FW=" + String(FW_VERSION)' in w, 'CTRL and W1P publish actual firmware identity for Setup')
