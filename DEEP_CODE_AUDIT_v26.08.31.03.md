# HV P2P v26.08.31.03 — Deep Code Audit
...
2. `fw_handle_block()` now accepts a mutable `Frame&`, allowing ESP32 Arduino Core 3.3.8 `Update.write(uint8_t*, size_t)` to receive the parser-owned mutable payload directly, with no const-cast and no copy.
